...
#[cfg(test)]
mod fix_database_test {
    use std::path::PathBuf;
    use crate::parsers::{ClaudeParser, LogParser};
    use crate::storage::Database;

    #[test]
    fn test_fix_database_directly() {
        let target_session_id = "4fe9325e-4c69-4633-ac6f-d879ca16d6c5";
        ...
        // 解析正确的文件
        let claude_parser = ClaudeParser::new();
        let session = claude_parser.parse_file(file_path).expect("Failed to parse file");
        
        println!("Parsed session {} with {} messages", session.id, session.messages.len());
        
        // 检查第一条消息
        let first_text = match &session.messages[0].content_blocks.first() {
            Some(crate::models::ContentBlock::Text { text }) => text.clone(),
            _ => String::new(),
        };
        println!("First message: {}", first_text.chars().take(50).collect::<String>());
        assert!(!first_text.contains("Warmup"), "Should not be Warmup");

        // 连接数据库并更新
        let db = Database::new(&db_path).expect("Failed to open database");
        
        // 检查当前状态
        let current_session = db.get_session(target_session_id)
            .expect("Failed to get session");
        
        if let Some(curr) = &current_session {
            println!("Current session has {} messages", curr.messages.len());
            let curr_first = match &curr.messages[0].content_blocks.first() {
                Some(crate::models::ContentBlock::Text { text }) => text.clone(),
                _ => String::new(),
            };
            println!("Current first message: {}", curr_first.chars().take(50).collect::<String>());
        }

        // 更新数据库
        db.update_session(&session).expect("Failed to update session");
        println!("Updated session in database");

        // 验证更新
        let updated_session = db.get_session(target_session_id)
            .expect("Failed to get updated session")
            .expect("Session not found");
        
        println!("Updated session has {} messages", updated_session.messages.len());
        let updated_first = match &updated_session.messages[0].content_blocks.first() {
            Some(crate::models::ContentBlock::Text { text }) => text.clone(),
            _ => String::new(),
        };
        println!("Updated first message: {}", updated_first.chars().take(50).collect::<String>());

        assert_eq!(updated_session.messages.len(), 12, "Should have 12 messages");
        assert!(!updated_first.contains("Warmup"), "Should not contain Warmup");
