...
mod tests {
    ...
    #[test]
    fn test_cursor_parse_http_servers() {
        let adapter = CursorAdapter;
        let content = r#"{
            "mcpServers": {
                "local-server": {
        ...
        // 验证 HTTP 服务
        let remote = services.iter().find(|s| s.name == "remote-server").unwrap();
        ...
        let http = services.iter().find(|s| s.name == "http-server").unwrap();
        assert_eq!(http.transport_type, crate::models::mcp::McpTransportType::Http);
        assert_eq!(http.url, Some("https://mcp.deepwiki.com/mcp".to_string()));
    }
